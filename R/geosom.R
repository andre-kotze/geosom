# reimplementation of supersom

geosom <- function(data,
                     grid = somgrid(),
                     rlen = 100,
                     alpha = c(0.05, 0.01),
                     ...
                     geo_weight = NULL)
{
  ...
  map_size = (max(geo_x) - min(geo_x)**2 + (max(geo_y) - min(geo_y))**2)**0.5
  geo_limit = map_size * geo_weight

  data = data[,!(names(data) %in% c(x_coord,y_coord))]
