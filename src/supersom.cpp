...
Rcpp::List RcppSupersom(
  ...
  int numEpochs,
  Rcpp::NumericMatrix geo_x,
  Rcpp::NumericMatrix geo_y,
  Rcpp::NumericVector geo_limit
  )
{
  int
  ...
  /* HERE PUT SOME TEST CODE 
     HERE PUT SOME TEST CODE 
       HERE PUT SOME TEST CODE 
  NEED TO DETERMINE MAP SIZE*/
  if (*pGeoLimit == 0) {
        /* then it is not a geosom, so don't filter BMU candidates*/
    pGeoLimit -= 1;
  }
  ...
  for (m = 0; m < numEpochs; m++) {
    ...
    for (k = 0; k < numObjects; k++) {
      ...
      /* somewhere here the magic happens:*/
